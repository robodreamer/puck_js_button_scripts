    ...
    clickTimer = setTimeout(function() {
        ...
        switch (clickCount) {
            case 1:
                ...
                NRF.sendHIDReport([0, 0, kb.KEY.A, 0, 0, 0, 0, 0], function() {
                    btnReleased(); // Release 'a' after pressing
                });
                handleLEDForSingleClick();
                break;
            case 2:
                // Double click, toggle LED2 and send 'b' if off or 's' if on
                toggleLED2();
                var key = isLED2On ? kb.KEY.B : kb.KEY.S;
                NRF.sendHIDReport([0, 0, key, 0, 0, 0, 0, 0], function() {
                    btnReleased(); // Release the key after pressing
                });
                break;
            case 3:
                // Triple click, output 'd' only if LED2 is on
                if (isLED2On) {
                    NRF.sendHIDReport([0, 0, kb.KEY.D, 0, 0, 0, 0, 0], function() {
                        btnReleased(); // Release 's' after pressing
                    });
                    turnOffLED2();
                }
                break;
